...
void GPUGLM_computeBlock<FPTYPE>::computeDerivatives(const GPUGLM_computeOptions<FPTYPE> * opts, const bool isSparseRun) {
    ...
    if(opts->compute_d2K) {
        ...
        FPTYPE beta  =  0;
        /*ce = cublasSYRK(cublasHandle2,
                        CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_T, ////uplo = CUBLAS_FILL_MODE_UPPER, CUBLAS_FILL_MODE_LOWER
                        dataset->lambda->getSize(1), dataset->lambda->getSize(0),
                        &alpha,
                        dataset->lambda->getData_gpu(), dataset->lambda->getLD_gpu(),
                        &beta,
                        results->d2K->getData_gpu(), results->d2K->getLD_gpu()); // This operation is usually way slower than GEMM even though it's specialized*/
