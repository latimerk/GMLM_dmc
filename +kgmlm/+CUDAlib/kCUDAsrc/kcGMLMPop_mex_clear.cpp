/*
 * kcGMLMPop_mex_clear.cu
 * Mex function to clear a GMLMPop object from the GPU.
 *  Takes two arguments: ptr (in long int form) to GMLMPop object
...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        // check args: arg 0 is GMLMPop object
        if(inputs.size() != 2) {
            matlabPtr->feval(u"error", 0,
                std::vector<matlab::data::Array>({ factory.createScalar("Two inputs required to kcGMLMPop_mex_clear!") }));
        }
        ...
        if(gpu_ptr_a.getNumberOfElements() < 1 || gpu_ptr_a[0] == 0) {
            ...
            //converts pointer to kCUDA::GPUGMLMPop and deletes
            if(isDouble[0]) {
                kCUDA::GPUGMLMPop<double> * gmlm = reinterpret_cast<kCUDA::GPUGMLMPop<double>*>(gpu_ptr);
                delete gmlm;
            }
            else {
                kCUDA::GPUGMLMPop<float > * gmlm = reinterpret_cast<kCUDA::GPUGMLMPop<float >*>(gpu_ptr);
                delete gmlm;
            }
        }
    }
