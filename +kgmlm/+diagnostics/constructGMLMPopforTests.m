function [GMLMstructure, trials] = constructGMLMPopforTests( factor_idx_0, isShared, isSharedEye, dim_A)
dim_R_max = 4;
...
trials = struct('Y', cell(dim_M,1), 'X_lin', [],  'Groups', []);
for mm = 1:dim_M
    dim_N = randi([dim_N_min, dim_N_max], 1);
    
    trials(mm).Y = randi([0,2], [dim_N dim_P]);
    trials(mm).X_lin = randn([dim_N dim_B dim_P])./sqrt(dim_B);
