function [accepted,err,paramStruct_new,log_p_accept,resultStruct] = hmcStep(obj,paramStruct_0,M_chol,HMC_state,resultStruct_0)
    
    %% generate initial momentum
    momentumStruct_0   = struct();
    momentumStruct_0.W = generateMomentum(numel(paramStruct_0.W),M_chol.W);
    
    if(~isempty(paramStruct_0.H))
        momentumStruct_0.H = generateMomentum(numel(paramStruct_0.H),M_chol.H);
    end
...
function [lp] = logProbMomentum(momentumStruct,paramStruct,M_chol)
    ...
    lp = lp + logProbMomentum_standard(momentumStruct.W,M_chol.W);
    if(~isempty(paramStruct.H))
        lp = lp + logProbMomentum_standard(momentumStruct.H,M_chol.H);
    end
...
function [paramStruct,momentumStruct,errs] = paramStep(paramStruct, momentumStruct, M_chol,HMC_state, useSinglePrecsion)
    ...
    p_new = standardStep(paramStruct.W,momentumStruct.W,HMC_state.stepSize.e,M_chol.W);
    ...
    if(~isempty(paramStruct.H))
        p_new = standardStep(paramStruct.H,momentumStruct.H,HMC_state.stepSize.e,M_chol.H);
        if(~all(~isnan(p_new) & ~isinf(p_new)))
            errs = true;
            return;
        end
...
function [momentumStruct,errs] = momentumStep(paramStruct, momentumStruct, resultStruct, HMC_state)
    ...
    % standard step for W
    m_new = standardMomentumStep(momentumStruct.W,resultStruct.dW,HMC_state.stepSize.e);
    if(~all(~isnan(m_new) & ~isinf(m_new)))
        errs = true;
        return;
    end
    momentumStruct.W  = m_new;

    % standard step for H
    if(~isempty(paramStruct.H))
        m_new = standardMomentumStep(momentumStruct.H,resultStruct.dH,HMC_state.stepSize.e);
        if(~all(~isnan(m_new) & ~isinf(m_new)))
            errs = true;
            return;
        end
        momentumStruct.H  = m_new;
    end
end
