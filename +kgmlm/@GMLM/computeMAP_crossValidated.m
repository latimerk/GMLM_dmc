%% GMLM.computeMAP_crossValidated
%   Fits the MAP using cross validation. Cross-validation is done over trials, not individual bins.
%
%   inputs:
...
%
%       params_init : a param struct of the initial value of the optimization. If not given or empty, a random initialization point will be generated by GMLM.getRandomParamStruct
%                      If params_init is an array (of length K + fitAll), then it specifies an init point for each fit.
%                     NOTE: this function does NOT enforce the hyperparams to be the same for each fit!
%
%
...
function [params_map, results_test_map, results_train_map] = computeMAP_crossValidated(obj, foldIDs, params_init, varargin)
...
addRequired( p, 'params_init'     ,     @(aa)all(obj.verifyParamStruct(aa, 'includeHyperparameters', true)));
...
parse(p, foldIDs, params_init, varargin{:});
...
for kk = 1:(K + fitAll)
    ...
    %runs optimization
    [params_map(kk), results_train_map(kk)] = obj.computeMAP( params_init(kk), 'alternating_opt' , alternating_opt, ...
    ...
    if(kk <= K)
        ...
        results_test_map(kk) = obj.computeLogPosterior(params_map(kk), opts_empty);
