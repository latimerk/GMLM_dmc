function [results_group] = fixedGaussianPrior(params, results, groupNum, prior_setup)
if(isempty(params))
    results_group = 1;
    return;
end
...
for ss = 1:dim_S
    ...
    if(strcmpi(params_group.dim_names(ss), prior_setup.groupName))
        ...
        %% log hyperprior
        if(isfield(results_group, 'dH') && ~isempty(results_group.dH))
            [lp_H, dlp_H_hyper] = DMC.priors.halfTPrior(H, prior_setup.hyperprior.nu);
        else
            lp_H = DMC.priors.halfTPrior(H, prior_setup.hyperprior.nu);
        end
